...
export class WebUserController {
    ...
    @Post('/snipeSet')
    async snipeSet(@Body() data: { id: string, webid: number, widx: number, tokenAddress: string, amount: string, gasprice: string, slippage: string, multi: boolean, autobuy: boolean }, @Headers() header:any) {
        const csrf = header['x-csrf-token'];
        return await this.service.snipeSet(data, csrf);
    }

    @Post('/mirrorSetOne')
    async mirrorSetOne(@Body() data: { id: string, webid: number, widx: number, mirrorAddress: string, amount: string }, @Headers() header:any) {
        const csrf = header['x-csrf-token'];
    ...
    @Post('/limitSetOne')
    async limitSetOne(@Body() data: { id: string, webid: number, aidx: number, widx: number, limitAddress: string, amount: string, limitPrice: string }, @Headers() header:any) {
        const csrf = header['x-csrf-token'];
